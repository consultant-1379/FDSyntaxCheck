...
function getProductNumber {
        product=`cat ${WORKSPACE}/FdSyntaxCheckTool/build.cfg | grep $module | grep $branch | awk -F " " '{print $3}'`
}


function setRstate {

        revision=`cat ${WORKSPACE}/FdSyntaxCheckTool/build.cfg | grep $module | grep $branch | awk -F " " '{print $4}'`
...
function appendRStateToPlatformReleaseXml {
		...
		versionXml="${WORKSPACE}/workspace/mom-tool/src/resources/version/release.xml"
		...
		mv ${WORKSPACE}/FdSyntaxCheckTool/release.xml ${WORKSPACE}/FdSyntaxCheckTool/release.${rstate}.xml
